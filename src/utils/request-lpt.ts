...
import { baseApiLpt } from './env';
import { getToken } from '@/apis/lpt/mine';
 ...
 *********************************************** 权限相关 ***********************************************
 ***************************************************************************************************************** **/
export const tokenKey = `lpt-token-${process.env.BUILD_ENV}`;
       ...
       if (!isRefreshing) {
         ...
         try {
          ...
          const getTokenRes =   await getToken();
           const token = getTokenRes.accessToken;
           // token在请求实例存储
           requestInstance.setConfig({ header: {
           ...
           oldConfig.header.Authorization = token;
