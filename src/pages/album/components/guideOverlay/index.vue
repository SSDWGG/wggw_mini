...
<template>
  <nut-overlay v-model:visible="account.showGuidePop" :z-index="2000" :close-on-click-overlay='false'
...
const next = () => {
  if(data.current < data.imageList.length - 1 ){
    ...
    Taro.setStorageSync('showGuidePop',false);
    account.showGuidePop = false;
  }
};


watch(toRef(account, 'showGuidePop'), (val: boolean) => {
  nextTick(() => {
